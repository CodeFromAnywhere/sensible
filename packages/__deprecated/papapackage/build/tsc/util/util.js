...
export const hasDependency = (packageJson, dependency) => {
 ...
 */
export function searchRecursiveSync(dir, ignore, match) {
...
export const findPackageDependencyPair = (dependencyPackagesNames) => (p) => {
    return {
        package: p,
        dependencies: unique(getAllPackageJsonDependencies(p), String).filter((dependency) => dependencyPackagesNames.includes(dependency)),
    };
};
export const getLinkingStrategy = (type) => {
...
export const getSrcDestsPairs = (argv, status) => {
...
export const calculateWatchlist = (argv) => {
    const status = ora({
        text: "Searching...",
        discardStdin: false,
        hideCursor: false,
    ...
    const numDestinations = watchlist.reduce((num, pair) => pair.dests.length + num, 0);
...
export const watchAndSubscribe = (client, debug, status, watchlist) => {
    return function (error, resp) {
        ...
        const numWatchDestinations = watchlist.reduce((num, watch) => watch.dests.filter((d) => d.linkingStrategy === "copy").length + num, 0);
...
export const linkWatchlist = (watchlist, cli, debug) => {
    const status = ora({
        text: "Linking...",
        discardStdin: false,
        hideCursor: false,
    ...
    const commands = watchlist.reduce((commands, watch) => {
        return [
            ...
            ...watch.dests
                .map((dest, index) => dest.linkingStrategy === "link"
                ? {
...
export const logWatchlist = (watchlist) => {
    console.log(colors.green("Watch list:"));
    watchlist.forEach((watch) => {
        console.log(colors.blue(`Source: ${watch.src.dependencyName} (${watch.src.folderPath})`));
        console.table(watch.dests.map((dest, index) => ({
...
export const getRelevantPackageInfo = (path) => {
    let fileBuffer;
    try {
 ...
 */
export const isHigherVersion = (x, y) => {
...
export const keepHighestVersion = (packages, current) => {
...
export const watchWatch = (client, status, debug) => (watch) => {
    createWatchmanConfig(watch);
    client.command(["watch-project", watch.src.folderPath], function (error, resp) {
        if (error) {
...
export function makeSubscription(client, watchBaseFolder, watchRelativePath, debug) {
    const sub = {
    ...
    const subName = `papapackage:${watchBaseFolder}${watchRelativePath ? `:${watchRelativePath}` : ""}`;
...
export const createSubscriptionEventEmitter = (client, watchlist, debug) => {
    ...
    client.on("subscription", function (resp) {
        ...
        if (watch) {
            const filteredDests = watch.dests.filter((dest) => dest.linkingStrategy === "copy");
            const linkedDests = watch.dests.filter((dest) => dest.linkingStrategy === "link");
            ...
            if (debug) {
                console.log(colors.green("Event"), `Copying ${filteredFiles.length} file(s) to ${filteredDests.length} destination(s)`, { fullPath }, colors.yellow("names: "), filteredFiles.map((f) => f.name), colors.blue("destinations: "), filteredDests.map((d) => path.join(d.folderPath, "node_modules", watch.src.dependencyName)));
            }
            const message = `${filteredFiles.length} ${filteredFiles.length === 1 ? "file" : "files"} changed. Copying into ${filteredDests.length} ${filteredDests.length === 1 ? "destination" : "destinations"} and linking into ${linkedDests.length} ${linkedDests.length === 1 ? "destination" : "destinations"}`;
            ...
            LATEST_EVENT_INFO = {
                ...
                copyDestinations: filteredDests.map((d) => path.join(d.folderPath, "node_modules", watch.src.dependencyName)),
                linkDestinations: linkedDests.map((d) => path.join(d.folderPath, "node_modules", watch.src.dependencyName)),
            };
            filteredFiles.forEach((file) => {
                ...
                filteredDests.map((dest) => {
                    ...
                    const to = path.join(dest.folderPath, "node_modules", watch.src.dependencyName, file.name);
...
export const createWatchmanConfig = (watch) => {
    ...
    try {
        ...
        fs.writeFileSync(watchmanConfigPath, JSON.stringify({ ignore_dirs: mustIgnore }));
    }
};
export const chooseFolder = (args) => {
    let folder = process.cwd();
    if (args[0]) {
        if (!fs.existsSync(String(args[0])) ||
...
export function unique(a, getId) {
    var seen = {};
    var out = [];
    var len = a.length;
    var j = 0;
...
export const getAllPackageJsonDependencies = (p) => {
...
export const getDependenciesList = (concatDependencies, p) => {
